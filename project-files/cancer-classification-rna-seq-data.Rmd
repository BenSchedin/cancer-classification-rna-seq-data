...
# TODO: update to use "full" rather than separate combination for svm
# Setting chunk seed
set.seed(123)
...
# Combining pca8 data with sample labels
ldaData <- bind_cols(pca8, labels)

# Splitting data into training and test sets with 75-25 split
trainIndices <- sample(1:nrow(ldaData), floor(0.75*nrow(ldaData)), replace=FALSE)

trainData <- ldaData[trainIndices, ]
testData <- ldaData[-trainIndices, ]
...
gmmData <- pca8
gmmData$Class <- labels$Class
gmmData <- reshape2::melt(gmmData, id="Class")
...
# Combining pca8 data with sample labels
qdaData <- bind_cols(pca8, labels)

# Splitting data into training and test sets with 75-25 split
trainIndices <- sample(1:nrow(qdaData), floor(0.75*nrow(qdaData)), replace=FALSE)

trainData <- qdaData[trainIndices, ]
testData <- qdaData[-trainIndices, ]
...
nnData$Class <- as.factor(nnData$Class)
