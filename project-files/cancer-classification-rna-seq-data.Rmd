...
geneScores <- as.data.frame(pca$x[, 1:8])
geneScores$Class <- as.factor(labels$Class)

# Splitting data into training and test sets, 75-25 split
trainIndices <- sample(1:nrow(geneScores), 0.75*nrow(geneScores), replace=FALSE)

train <- geneScores[trainIndices,]
test <- geneScores[-trainIndices,]
